...
| `src/PDv2*.{h,cpp}` | **PDv2 engine glue**: `PDv2Mgr` (config, plans, manifest, chunk-meta walk masks), `PDv2InstanceScript` (spawns, fall catcher, per-instance walk grid), `PDv2CreatureAI` (grid chase / MovePoint chains; deliberately NO leash — dungeon mobs chase until the target is gone) + the `AllCreatureScript::GetCreatureAI` binder that attaches it to stock templates on the v2 map, `PDv2MapScript`, `.pdungeon v2` commands |
| `tests/blockplan_harness.cpp` | Standalone verification of the block planner + walk grid + approach policy + world math: `g++ -std=c++17 -Wall -Wextra -Werror -O2 -Isrc tests/blockplan_harness.cpp src/generator/PDBlockPlan.cpp src/generator/PDv2WalkGrid.cpp -o pdblock`; `pdblock <seed> [rooms]` / `pdblock --batch 500 [rooms]` / `pdblock --path <seed> [rooms]` / `pdblock --manifest <seed> <file> [rooms]` |
