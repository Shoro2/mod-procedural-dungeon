...
| `data/sql/db-world/` | `instance_template` + `map_dbc` override (base map), GO/creature templates (910000+/910500+), `pdungeon_palette`; PDv2: `mod_pdungeon_map760.sql` (generated by workspace script 47) + `mod_pdungeon_chunk_meta.sql` (generated by script 48 — **regenerate, never hand-edit**) |
| `data/sql/db-characters/` | `pdungeon_runs` history; `pdungeon_account` (PDv2 persistence — layout as seed + generation inputs, regenerated on login; layout columns updated via ON DUPLICATE KEY so dlvl/dxp are never clobbered) |
