...
| `src/PDv2*.{h,cpp}` | **PDv2 engine glue**: `PDv2Mgr` (config, plans, manifest, chunk-meta walk masks), `PDv2InstanceScript` (spawns, fall catcher, per-instance walk grid), `PDv2CreatureAI` (grid chase / MovePoint chains / leash) + the `AllCreatureScript::GetCreatureAI` binder that attaches it to stock templates on the v2 map, `PDv2MapScript`, `.pdungeon v2` commands |
