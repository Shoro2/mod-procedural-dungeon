...
| **D5** | Class fit | Rolled gear is filtered to the looter: `AllowableClass` **and `AllowableRace`** (the T9 sets exist per faction), armour type = the class's highest (plate/mail/leather/cloth), shields for war/pal/sha, a static class→weapon-subclass table; relics via `AllowableClass`. **Stat profile filtered since WP9** when the looter owns *Discerning Eye* (aura tag 76004) and chose a profile in the panel (Strength / Agility / Caster): primary stat wins, zero-stat items pass, a no-primary item is judged by its evidence. Empty filtered pool → the profile is dropped first, then the class (two-stage fallback). `V2.Loot.ClassFilter = 1` | Pure random from 1 062 items is mostly disenchant fodder |
