...
| D5 | Mood | **Operator 2026-09-11: every theme gets its OWN light, ambience and music** (was: shared). Lever per recon `roundf-recon-d.md`: per-theme MCNK areaid written by script 48 -> one AreaTable row per theme (ZoneMusic, AmbienceID, Lightid) + per-theme Light rows; a client DBC patch is part of the kit-v40 delivery. Planned as package F3. |
