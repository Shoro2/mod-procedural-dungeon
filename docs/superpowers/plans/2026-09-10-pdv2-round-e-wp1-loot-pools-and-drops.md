...
- [ ] **Step 1b: Tier-set pieces as a seventh source (operator 2026-09-10, spec D4).** Set pieces never sit in a boss loot table (they are token-bought), so the script adds them directly: `SELECT entry, name, ItemLevel, itemset, AllowableRace FROM item_template WHERE Quality = 4 AND class = 4 AND InventoryType <> 0 AND itemset <> 0 AND RequiredLevel = 80 AND ItemLevel IN (200, 213, 219, 226, 232, 245, 258, 251, 264, 277) AND NOT (<the resilience test>)`, then assign by ilvl: 200/213/219/226/232/245 → `RAID_N`, 258 → `RAID_HC`, 251/264 → `ICC_N`, 277 → `ICC_HC`; `category = 'tier'`, weight 100, comment `"<name> ilvl <n> set <itemset>"`. Report the count per pool and per ilvl in `--check` (expectation: roughly 19 sets × 5 pieces per ilvl, T9 doubled by faction — verify and pin what you measure; anything wildly off means the filter caught a non-tier set, so print the distinct `itemset` ids with their piece counts and drop any set with fewer than 5 pieces). Insert them with the same `DELETE`/`INSERT` per pool (one DELETE per pool covers both sources).
